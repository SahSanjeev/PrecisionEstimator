...
# Exchange rate: 1 USD = 94.8358 INR (as of May 2, 2026)
USD_TO_INR_RATE = 94.8358
...
class PriceEstimatorApp:
    ...
    def update_services_listbox(self):
        ...
        for idx, s in enumerate(self.services_list, 1):
            if s.get("num_sheets"):
                self.services_listbox.insert("end", f"{idx}. {s['category']} - {s['service']} | {s['quantity']} sqft ({s['num_sheets']} sheets) | Rate: ${s['average_price']:.2f} (₹{s['average_price'] * USD_TO_INR_RATE:.2f}) | Subtotal: ${s['estimated_cost']:.2f} (₹{s['estimated_cost'] * USD_TO_INR_RATE:.2f})")
            else:
                self.services_listbox.insert("end", f"{idx}. {s['category']} - {s['service']} | Qty: {s['quantity']} {s['unit']} | Rate: ${s['average_price']:.2f} (₹{s['average_price'] * USD_TO_INR_RATE:.2f}) | Subtotal: ${s['estimated_cost']:.2f} (₹{s['estimated_cost'] * USD_TO_INR_RATE:.2f})")
    ...
    def update_total_label(self):
        ...
        self.total_label.config(text=f"Total: ${total:,.2f} (₹{total * USD_TO_INR_RATE:,.2f})")
    ...
    def export_to_pdf(self):
        ...
        for s in self.services_list:
            pdf.cell(0, 6, txt=f"- {s['category']} | {s['service']} | Qty: {s['quantity']} {s['unit']} | Rate: ${s['average_price']:.2f} (₹{s['average_price'] * USD_TO_INR_RATE:.2f}) | Subtotal: ${s['estimated_cost']:.2f} (₹{s['estimated_cost'] * USD_TO_INR_RATE:.2f})", ln=True)
        pdf.ln(8)
        pdf.set_font("Arial", 'B', 14)
        pdf.cell(0, 10, txt=f"Total Estimate: ${self.calculate_total():,.2f} (₹{self.calculate_total() * USD_TO_INR_RATE:,.2f})", ln=True)
        pdf.ln(5)
        pdf.set_font("Arial", 'I', 8)
        pdf.cell(0, 6, txt=f"Exchange rate used: 1 USD = {USD_TO_INR_RATE} INR (as of May 2, 2026)", ln=True, align="L")
        pdf.ln(10)
